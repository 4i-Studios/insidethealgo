...
class ModifiedBubbleSortWidgets {
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: [
            ...
            if (logic.isSorting) ...[
              const SizedBox(width: 8),
              _buildSwappedBadge(logic.currentSwapped),
            ],
          ],
        ),
      ),
  ...
  Widget _buildSwappedBadge(bool value) {
    return Container(
      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
      decoration: BoxDecoration(
  ...
  Widget _buildCodeDisplay(BuildContext context) {
    ...
    List<Map<String, dynamic>> codeLines = [
      {'line': 0, 'text': 'void modifiedBubbleSort(List<int> arr, bool ascending) {', 'indent': 0},
      {'line': 1, 'text': '  for (int i = $iValue; i < $nValue - 1; i++) {', 'indent': 1},
      {'line': 2, 'text': '    bool swapped = false;', 'indent': 2},
      {'line': 3, 'text': '    for (int j = $jValue; j < $nValue - $iValue - 1; j++) {', 'indent': 2},
      {'line': 4, 'text': '      if ($arrJ $compareOp $arrJplus1) {', 'indent': 3},
      {'line': 5, 'text': swapText, 'indent': 4},
      {'line': 6, 'text': '        swapped = true;', 'indent': 4},
      {'line': 7, 'text': '      }', 'indent': 3},
      {'line': 8, 'text': '    }', 'indent': 2},
      {'line': 9, 'text': '    // Check if any swaps were made', 'indent': 2},
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16),
          const Text('ModifiedBubbleSort.dart', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
  ...
  Color _getCodeTextColor(String text) {
    if (text.contains('void') || text.contains('for') || text.contains('if') || text.contains('bool')) {
      return const Color(0xFF569CD6);
    } else if (text.contains('arr[') || text.contains('swap') || text.contains('swapped')) {
