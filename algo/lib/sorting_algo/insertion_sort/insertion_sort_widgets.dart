...
class InsertionSortWidgets {
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: [
            ...
            _buildColorLegendItem(Colors.purple, 'Key'),
            const SizedBox(width: 16),
            _buildColorLegendItem(Colors.orange, 'Comparing'),
            const SizedBox(width: 16),
            _buildColorLegendItem(Colors.red, 'Inserting'),
            const SizedBox(width: 16),
    ...
    return Container(
      ...
      child: SingleChildScrollView(
        ...
        child: Row(
          ...
          children: [
            _buildStatusChip('Pass', '${logic.currentI}', Colors.blue),
            const SizedBox(width: 8),
            _buildStatusChip('Key', '${logic.keyElement >= 0 ? logic.keyElement : '-'}', Colors.purple),
            const SizedBox(width: 8),
            _buildStatusChip('Comparisons', '${logic.totalComparisons}', Colors.orange),
            const SizedBox(width: 8),
            _buildStatusChip('Insertions', '${logic.totalInsertions}', Colors.red),
          ],
        ),
      ),
    );
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        return AnimatedBuilder(
          animation: logic.insertAnimation,
                ...
                child: Container(
                  ...
                  child: Row(
                    ...
                    children: logic.numbers.asMap().entries.map((entry) {
                      ...
                      double offset = 0;
                      if (logic.isInserting && index == logic.keyIndex) {
                        offset = -20 * logic.insertAnimation.value;
                      }
                        ...
                        child: Transform.translate(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              SizedBox(height: _getResponsiveSize(context, defaultSize: 4, minSize: 2)),
                              Container(
                                width: _getResponsiveSize(context, defaultSize: 30, minSize: 25),
                                height: barHeight,
                                decoration: BoxDecoration(
                                  color: logic.getBarColor(index),
  ...
  Widget _buildCodeDisplay(BuildContext context) {
    ...
    String keyValue = logic.keyElement >= 0 ? '${logic.keyElement}' : 'arr[i]';
    ...
    List<Map<String, dynamic>> codeLines = [
      {'line': 0, 'text': 'void insertionSort(List<int> arr, bool ascending) {', 'indent': 0},
      {'line': 1, 'text': '  for (int i = $iValue; i < $n; i++) {', 'indent': 1},
      {'line': 2, 'text': '    int key = $keyValue;', 'indent': 2},
      {'line': 3, 'text': '    int j = $jValue;', 'indent': 2},
      {'line': 4, 'text': '    while (j >= 0 && $arrJ $compareOp key) {', 'indent': 2},
      {'line': 5, 'text': '      arr[j + 1] = arr[j];', 'indent': 3},
      {'line': 6, 'text': '      j--;', 'indent': 3},
      {'line': 7, 'text': '    }', 'indent': 2},
      {'line': 8, 'text': '    arr[j + 1] = key;', 'indent': 2},
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 16),
          const Text('InsertionSort.dart', style: TextStyle(color: Colors.white, fontSize: 13, fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
  ...
  Color _getCodeTextColor(String text) {
    if (text.contains('void') || text.contains('for') || text.contains('while') || text.contains('int')) {
      return const Color(0xFF569CD6);
    } else if (text.contains('arr[') || text.contains('key') || text.contains('j++') || text.contains('j--')) {
